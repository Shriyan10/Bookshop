...
class ProductController extends BaseController
{
    ...
    function getAllProducts(int $start, int $limit, string $search): void
    {
        try {
            ...
            if (strlen($search) > 0) {
                $query = "SELECT COUNT(*) as quantity, bd.* from products b JOIN product_details bd ON bd.id=b.product_detail_id WHERE bd.title LIKE '%$search%' GROUP BY bd.id LIMIT " . $limit . " OFFSET " . $offset;
                $countQuery = "SELECT COUNT(*) as count FROM product_details WHERE title LIKE '%$search%'";
            } else {
                $query = "SELECT COUNT(*) as quantity, bd.* from products b JOIN product_details bd ON bd.id=b.product_detail_id GROUP BY bd.id LIMIT " . $limit . " OFFSET " . $offset;
